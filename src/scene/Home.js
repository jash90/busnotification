  ...
  saveTransport = () => {
    ...
    if (this.state.select != null) {
      ...
      this
        ...
        .doc
        .ref
        .update({time: this.state.time, direction: this.state.direction, transport});
      if (this.state.select.active) {
        PushNotification.cancelLocalNotifications({id: this.state.select.id});
        PushNotification.localNotificationSchedule({
          id: this.state.select.id, autoCancel: false, color: "#2196F3", // (optional) default: system default
          title: "Transport Notification", // (optional)
          message: this.state.direction + " " + Moment(this.state.time).format("HH:mm") + " " + transport,
